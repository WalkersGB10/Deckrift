...
var crystals_used = 0
var fates_used = 0
var destroyed_jokers = 0
var pljokers = []
var basedealers = [100, 300, 600, 1000, 2400, 6000, 12000, 25000, 50000]
var handvalues = {
  4: [5, 1],
  5: [5, 1],
  6: [5, 1],
...
func _setupdecks():
	...
	for suit in suits:
		for value in values:
			decks["standard"].append(value+suit)  
			
	#noface
	for suit in suits:
		for value in values:
			if value not in ["J", "Q", "K"]:
				decks["noface"].append(value+suit)
	
	#even
	for suit in suits:
		for value in ["A", "2", "4", "6", "8", "T", "J", "Q", "K"]:
			decks["even"].append(value + suit)

  #odd
  for suit in suits:
    for value in ["A", "3", "5", "7", "9", "J", "Q", "K"]
      decks["odd".append(value + suit)
